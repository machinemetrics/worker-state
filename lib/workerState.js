...
function WorkerState (table, app, shard, dynamodb) {
  this.state = {};
  this.substate = {};
  this.table = new WorkerStateTable(table, dynamodb);
  this.app = app;
  this.shard = shard;
  this.checkpoint = '';
...
WorkerState.prototype.initializeKey = function (key) {
  ...
  return this.table.queryItems({
    ...
    _.each(data.Items, function (item) {
      var key = self.parseAppKey(item['app.key']);
      var subkey = self.parseShardKey(item['shard.key']);
...
WorkerState.prototype.writeToWorkerState = function (key, subkey, value, previous, seq, timestamp) {
  ...
  return this.table.writeItem(item);
};
...
WorkerState.prototype.deleteAllFromWorkerState = function (key) {
  ...
  return this.table.queryItems({
    ...
    var records = _.map(data.Items, function (item) {
      return {
        Key: {
    ...
    return self.table.deleteItems(records).then(function () {
...
WorkerState.prototype.mergeShards = function (shardLeft, shardRight) {
  var self = this;
  var stateLeft = new WorkerState(self.table.tableName, self.app, shardLeft);
  var stateRight = new WorkerState(self.table.tableName, self.app, shardRight);
...
WorkerState.prototype.splitShard = function (shardParent) {
  var self = this;
  var stateParent = new WorkerState(self.table.tableName, self.app, shardParent);
  ...
  return stateParent.initialize(self.initKeys).then(function () {
    ...
    return stateParent.table.updateItem({
      Key: {
