...
RedisWorkerStore.prototype.readItems = function (key, subkey, partialMatch) {
  ...
  function zrangebylex(key, min, max) {
    const entry = self.logCommand('readItems', ['zrangebylex', key, min, max]);
    return self.redis.zrangebylexAsync(key, min, max).then((result) => {
      entry.result = _.clone(result);
      return result;
    });
  }

  function mget(keys) {
    const entry = self.logCommand('readItems', ['mget', keys]);
    return self.redis.mgetAsync(keys).then((result) => {
      entry.result = _.clone(result);
      return result;
    });
  }
  ...
  function get(key) {
    const entry = self.logCommand('readItems', ['get', key]);
    return self.redis.getAsync(key).then((result) => {
      entry.result = result;
      return result;
    });
  }
...
RedisWorkerStore.prototype.writeAttributeIfNotExists = function (key, subkey, attr, value) {
  ...
  function get(key) {
    const entry = self.logCommand('writeAttributeIfNotExists', ['get', key]);
    return self.redis.getAsync(key).then((result) => {
      entry.result = result;
      return result;
    });
  ...
  function set(key, value) {
    ...
    return self.redis.setAsync(key, value);
  }
};
...
RedisWorkerStore.prototype.logCommand = function (domain, command) {
  if (_.isArray(this.commandLog)) {
    ...
    const entry = { command: `${domain}: ${command}` };
