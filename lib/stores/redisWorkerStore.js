...
RedisWorkerStore.prototype.readItems = function (key, subkey, partialMatch) {
  ...
  function zrangebylex(key, min, max) {
    self.logCommand('readItems', ['zrangebylex', key, min, max]);
    return self.redis.zrangebylexAsync(key, min, max);
  }

  function mget(keys) {
    self.logCommand('readItems', ['mget', keys]);
    return self.redis.mgetAsync(keys);
  }

  function get(key) {
    self.logCommand('readItems', ['get', key]);
    return self.redis.getAsync(key);
  }
};
...
RedisWorkerStore.prototype.writeAttributeIfNotExists = function (key, subkey, attr, value) {
  ...
  function get(key) {
    self.logCommand('writeAttributeIfNotExists', ['get', key]);
    return self.redis.getAsync(key);
  }

  function set(key, value) {
    self.logCommand('writeAttributeIfNotExists', ['set', value]);
