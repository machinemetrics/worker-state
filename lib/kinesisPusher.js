...
KinesisPusher.prototype.pushMarkerToShards = function (shards, marker, seq) {
  ...
  return pushRecordBatch(self.kinesis, self.stream, data);
};

function pushRecordBatch (kinesis, stream, records) {
  var backoff = 25;
  var shardMaxSequence = {};
  ...
  return async.until(function () {
    ...
    return kinesis.putRecords({
      ...
      if (resp.FailedRecordCount) {
        records = records.concat(processed[1]);
        backoff = Math.min(backoff * 2, 1000);
        return Q.delay(backoff);
      }
    });
