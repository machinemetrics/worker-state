...
function KinesisPusher (workerState, stream, kinesis) {
  ...
  this.options = {
    maxPackedRecordCount: undefined,
...
KinesisPusher.prototype.pushRecords = function (records, seq) {
  ...
  return self.workerState.readFromWorkerState(self.appKey(), KinesisPusher.pushKey, seq).then(function (data) {
    if (data && bigInt(data.sequence).leq(bigInt(seq)))
    ...
    records = self.util.groupAndPackRecords(records, self.options.maxPackedRecordCount);
    return self.util.pushRecords(self.stream, records, self.options).then(function (shardMaxSequence) {
...
KinesisPusher.prototype.pushRecordsWithoutRecovery = function (records) {
  var self = this;
  ...
  return self.logEvent('StartCullPrevious', {
    ...
    return self.pushMarkerToShards(writeableShards, '__MARKER', self.workerState.checkpoint).then(function (markerSeqs) {
      return async.each(shardPartitions, function (shardId) {
        ...
        return self.kinesis.getShardIterator(iteratorParams).promise().then(function (data) {
