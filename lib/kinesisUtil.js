...
KinesisUtil.prototype.groupAndAggregateRecords = function(records, count) {
  count = count || 100;
...
KinesisUtil.prototype.pushRecords = function (stream, records, maxBatchSize) {
  ...
  return async.eachSeries(records, function (group) {
    return async.until(function () {
      ...
      var request = self.takeNextBatch(group, maxBatchSize); // _.take(data, 500);
      return self.kinesis.putRecords({
        ...
        records = _.drop(records, request.length);
        if (resp.FailedRecordCount) {
          records = records.concat(_.reject(request, function (rec, index) {
            return resp.Records[index].ShardId;
          }));
