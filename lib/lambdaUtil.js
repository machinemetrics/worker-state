...
var fs = require('fs');
var KinesisUtil = require('./kinesisUtil');
...
LambdaUtil.prototype.runLambdaFileHarness = async function (app, workerState, config) {
  ...
  const context = {
    ...
    getRemainingTimeInMillis: function () {
      const maxRuntime = config.timeout || moment.duration(1, 'day');
  ...
  const results = await loadData('input.csv');
  console.log(results);
};
  ...
  return Q.forever(function () {
    return Q().then(function () {
      ...
      if (config.timestamp) {
        ...
        params.StartingSequenceNumber = config.sequence;
